...
import './freeplay-pokemon-map.css';
import { LatLng, LatLngBounds } from 'leaflet';
import { Link, useMediaQuery, useTheme } from '@mui/material';
import { regionThemes } from '../../utils/region-themes';
import { Region, RegionSelector } from '../region-selector/region-selector';
import { KantoMap } from '../kanto/kanto-map';
import { JohtoMap } from '../johto/johto-map';
import { StyledSwitch } from '../switch/styled-switch';
...
const PokemonMap = () => {
    ...
    const [didRegionJustChange, setDidRegionJustChange] = useState(false);
    ...
    return (
        <>
            <Link sx={{ marginBlockStart: '8px', marginInlineStart: '8px' }} className="logo-container" href="/">
                <img className="logo" src="/images/logo.png" alt="PokeMelody logo" />
            </Link>
            ...
            <div className="stuff">
                ...
                <div className="buttons">
                    <button
                        className="button"
                        onClick={() => {
                            if (audioRef.current?.paused && !didRegionJustChange) {
                                audioRef.current.play();
                            } else {
                                setDidRegionJustChange(false);
                                playRandomSound();
                            }
                        }}
                    >
